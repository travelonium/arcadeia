    ...
    class VideoFile : MediaFile
    {
      ...
      /*      From      To       Count             Maximum Duration     Every X Seconds           */
           ...
           : base("Video", path)
        {
            ...
            if (Self != null)
            {
                // Initialize the Thumbnails. This will enumerate the already existing Thumbnail files.
                ...
                if (Created)
                {
                    ...
                    var jsonVideoFileInfo = GetVideoFileInfo(path);
        ...
        public VideoFile(XElement element, bool update = false)
           : base(element, update)
        {
            if (Self != null)
            {
                ...
                if (update)
                {
                    if (Modified)
                    {
                        ...
                        var jsonVideoFileInfo = GetVideoFileInfo(FullPath);
        ...
        private static Dictionary<string, dynamic> GetVideoFileInfo(string path)
        {
            string output = null;

            if (Properties.Settings.Default.FFMPEGLocation.Length == 0)
            {
                throw new DirectoryNotFoundException("ffmpeg's directory was not specified!");
            }

            using (Process ffprobe = new Process())
            {
                ffprobe.StartInfo.FileName = Properties.Settings.Default.FFMPEGLocation + "\\ffprobe.exe";
                ...
                ffprobe.WaitForExit(Properties.Settings.Default.GetVideoFileInfoTimeout);
            }

            var javaScriptSerializer = new JavaScriptSerializer();
            var jsonOutput = javaScriptSerializer.Deserialize<Dictionary<string, dynamic>>(output);

            return jsonOutput;
        }

        private static byte[] GenerateVideoThumbnail(string filePath, int position, int width, int index)
        {
            byte[] output = null;
            string fileName = Path.GetTempPath() + "\\" + System.IO.Path.GetRandomFileName() + ".jpg";
            ...
            {
                ffmpeg.StartInfo.FileName = Properties.Settings.Default.FFMPEGLocation + "\\ffmpeg.exe";
                ...
                while ((!ffmpeg.HasExited) &&
                       (totalWaitTime < Properties.Settings.Default.GenerateVideoThumbnailTimeout));
                                        ...
                                        IProgress<byte[]> preview)
        {
            ...
            for (int index = 1; index <= totalThumbnails; index++)
            {
                ...
                thumbnail = GenerateVideoThumbnail(FullPath, position, 512, index);
            ...
            // Re-Initialize the Thumbnails. This will enumerate the newly generated Thumbnail files.
            Thumbnail = new MediaContainerThumbnail(Id, Properties.Settings.Default.ThumbnailsCachingMethod);
        }
